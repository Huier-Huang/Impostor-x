...
internal class MatchmakerService : IHostedService
{
    ...
    {
        await _matchmaker.StartAsync(IPAddress.Parse(_serverConfig.ResolveListenIp()), _serverConfig.ListenPort);

        _logger.LogInformation(
            "Matchmaker is listening on {0}:{1}, the public server ip is {2}:{3}.",
            _serverConfig.ResolveListenIp(),
        ...
        {
            _logger.LogWarning(
                "Your HTTP server is exposed to the public internet, we recommend setting up a reverse proxy and enabling HTTPS");
            _logger.LogWarning(
